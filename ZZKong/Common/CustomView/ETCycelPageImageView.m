...
@interface ETCyclePageImageView () <UICollectionViewDelegate, UICollectionViewDataSource>

@property (strong, nonatomic) ETCollectionView *contentView;
...
@implementation ETCyclePageImageView

- (id)initWithFrame:(CGRect)frame
{
    self = [super initWithFrame:frame];
    if (self) {
        [self _ETPageView_setup];
    }
    return self;
}
...
- (void)_ETPageView_setup
{
    self.interval = 5.f;
    
    self.contentView = [[ETCollectionView alloc] initWithFrame:CGRectZero collectionViewLayout:[ETCyclePageImageViewLayout new]];
    [self addSubview:self.contentView];
    [self.contentView registerClass:[ETCyclePageImageViewCell class] forCellWithReuseIdentifier:[ETCyclePageImageViewCell cellReuseIdentifier]];
    [ETConstraintHelper setView:self.contentView fullAsSuperview:self];
...
{
    ETCyclePageImageViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:[ETCyclePageImageViewCell cellReuseIdentifier] forIndexPath:indexPath];
    
    WEAK_SELF;
    [cell updateWithImageUrlString:[self.dataSource pageView:self imageUrlStringAtIndex:indexPath.item % self.pageCount] andClickBlock:^(ETBlockImageView *sender) {
        STRONG_SELF;
    ...
    } andImageNilBlock:^(ETBlockImageView *sender) {
        STRONG_SELF;
