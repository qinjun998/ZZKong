...
#define ET_BASE_SDK_7_ENABLED
#endif

#ifdef DEBUG
#define ET_LOG(fmt, ...)		NSLog((@"%s (%d) : " fmt), __PRETTY_FUNCTION__, __LINE__, ##__VA_ARGS__)
#else
#define ET_LOG(...)
#endif

#define ET_IS_IOS_EQUAL_TO(v)                  ([[[UIDevice currentDevice] systemVersion] compare:v options:NSNumericSearch] == NSOrderedSame)

#define ET_IS_IOS_GREATER_THAN(v)              ([[[UIDevice currentDevice] systemVersion] compare:v options:NSNumericSearch] == NSOrderedDescending)

#define ET_IS_IOS_GREATER_THAN_OR_EQUAL_TO(v)  ([[[UIDevice currentDevice] systemVersion] compare:v options:NSNumericSearch] != NSOrderedAscending)

#define ET_IS_IOS_LESS_THAN(v)                 ([[[UIDevice currentDevice] systemVersion] compare:v options:NSNumericSearch] == NSOrderedAscending)


////////////////////////////////////////////////////////////////////////////////////////////////////////

@interface UIColor (ETPopover)
...
@implementation UIColor (ETPopover)
...
@interface UINavigationController (ETPopover)
...
@implementation UINavigationController (ETPopover)
...
@interface UIViewController (ETPopover)
@end

////////////////////////////////////////////////////////////////////////////////////////////////////////

@implementation UIViewController (ETPopover)
...
@interface ETPopoverArea : NSObject
{
}

@property (nonatomic, assign) ETPopoverArrowDirection arrowDirection;
...
#pragma mark
#pragma mark - ETPopoverArea

@implementation ETPopoverArea
...
- (NSString*)description
{
    ...
    if (arrowDirection == ETPopoverArrowDirectionUp)
    {
        direction = @"UP";
    }
    else if (arrowDirection == ETPopoverArrowDirectionDown)
    {
        direction = @"DOWN";
    }
    else if (arrowDirection == ETPopoverArrowDirectionLeft)
    {
        direction = @"LEFT";
    }
    else if (arrowDirection == ETPopoverArrowDirectionRight)
    {
        direction = @"RIGHT";
    }
    else if (arrowDirection == ETPopoverArrowDirectionNone)
    {
        direction = @"NONE";
    }
...
+ (id)theme {
    ...
    ETPopoverTheme *result = nil;
    
    if (ET_IS_IOS_LESS_THAN(@"7.0")) {
        result = [ETPopoverTheme themeForIOS6];
    } else {
        result = [ETPopoverTheme themeForIOS7];
    }
...
+ (id)themeForIOS6 {
    ...
    ETPopoverTheme *result = [[ETPopoverTheme alloc] init];
...
#pragma mark
#pragma mark - UIImage (ETPopover)

@implementation UIImage (ETPopover)
...
@interface ETPopoverBackgroundInnerView : UIView
...
#pragma mark
#pragma mark - ETPopoverInnerView

@implementation ETPopoverBackgroundInnerView
...
@protocol ETPopoverOverlayViewDelegate;

@interface ETPopoverOverlayView : UIView
{
    BOOL testHits;
}

@property(nonatomic, assign) id <ETPopoverOverlayViewDelegate> delegate;
...
#pragma mark
#pragma mark - ETPopoverOverlayViewDelegate

@protocol ETPopoverOverlayViewDelegate <NSObject>

@optional
- (void)popoverOverlayViewDidTouch:(ETPopoverOverlayView *)overlayView;
...
#pragma mark
#pragma mark - ETPopoverOverlayView

@implementation ETPopoverOverlayView
...
#pragma mark
#pragma mark - ETPopoverBackgroundViewDelegate

@protocol ETPopoverBackgroundViewDelegate <NSObject>

@optional
- (void)popoverBackgroundViewDidTouchOutside:(ETPopoverBackgroundView *)backgroundView;

@end

////////////////////////////////////////////////////////////////////////////////////////////////////////

@interface ETPopoverBackgroundView ()
{
    ETPopoverBackgroundInnerView *innerView;
    CGSize contentSize;
}

@property(nonatomic, assign) id <ETPopoverBackgroundViewDelegate> delegate;
...
- (CGRect)outerRect:(CGRect)rect arrowDirection:(ETPopoverArrowDirection)aArrowDirection;
- (CGRect)innerRect:(CGRect)rect arrowDirection:(ETPopoverArrowDirection)aArrowDirection;
- (CGRect)arrowRect:(CGRect)rect arrowDirection:(ETPopoverArrowDirection)aArrowDirection;
...
#pragma mark
#pragma mark - ETPopoverBackgroundView

@implementation ETPopoverBackgroundView
...
- (id)initWithContentSize:(CGSize)aContentSize
{
    ...
    if (self != nil)
    {
        ...
        self.arrowDirection = ETPopoverArrowDirectionDown;
        self.arrowOffset = 0;
        
        self.layer.name = @"parent";
        
        if (ET_IS_IOS_GREATER_THAN_OR_EQUAL_TO(@"6.0"))
        {
...
- (void)setArrowOffset:(float)value
{
    ...
    if (value != 0)
    {
        ...
        if (arrowDirection == ETPopoverArrowDirectionUp || arrowDirection == ETPopoverArrowDirectionDown)
        {
            value += coef * self.outerShadowOffset.width;
            value = MIN(value, CGRectGetWidth(outerRect) / 2);
        }
        
        if (arrowDirection == ETPopoverArrowDirectionLeft || arrowDirection == ETPopoverArrowDirectionRight)
        {
...
- (void)setViewController:(UIViewController *)viewController
{
    ...
    if (innerView == nil)
    {
        innerView = [[ETPopoverBackgroundInnerView alloc] initWithFrame:contentView.frame];
...
- (void)drawLayer:(CALayer *)layer inContext:(CGContextRef)context
{
    if ([layer.name isEqualToString:@"parent"])
    {
        ...
        else if (arrowDirection == ETPopoverArrowDirectionLeft || arrowDirection == ETPopoverArrowDirectionRight)
        {
            if (arrowOffset >= 0)
            {
        ...
        if (arrowDirection == ETPopoverArrowDirectionDown)
        {
        ...
        if (arrowDirection == ETPopoverArrowDirectionLeft)
        {
        ...
        if (arrowDirection == ETPopoverArrowDirectionRight)
        {
        ...
        if (arrowDirection == ETPopoverArrowDirectionNone)
        {
...
- (CGRect)outerRect:(CGRect)rect arrowDirection:(ETPopoverArrowDirection)aArrowDirection
{
    ...
    if (aArrowDirection == ETPopoverArrowDirectionUp || arrowDirection == ETPopoverArrowDirectionDown)
    {
        result.size.height -= arrowHeight;
        
        if (aArrowDirection == ETPopoverArrowDirectionUp)
        {
            result = CGRectOffset(result, 0, arrowHeight);
        }
    }
    
    if (aArrowDirection == ETPopoverArrowDirectionLeft || arrowDirection == ETPopoverArrowDirectionRight)
    {
        result.size.width -= arrowHeight;
        
        if (aArrowDirection == ETPopoverArrowDirectionLeft)
        {
...
- (CGRect)arrowRect:(CGRect)rect arrowDirection:(ETPopoverArrowDirection)aArrowDirection
{
    ...
    if (arrowHeight > 0)
    {
        ...
        if (aArrowDirection == ETPopoverArrowDirectionDown)
        {
        ...
        if (aArrowDirection == ETPopoverArrowDirectionRight)
        {
...
@interface ETPopoverController () <ETPopoverOverlayViewDelegate, ETPopoverBackgroundViewDelegate>
{
    UIViewController        *viewController;
    CGRect                   rect;
    UIView                  *inView;
    ETPopoverOverlayView    *overlayView;
    ETPopoverBackgroundView *backgroundView;
    ETPopoverArrowDirection  permittedArrowDirections;
    BOOL                     animated;
    ...
    ETPopoverAnimationOptions options;
...
- (void)dismissPopoverAnimated:(BOOL)aAnimated
                       options:(ETPopoverAnimationOptions)aAptions
                    completion:(void (^)(void))aCompletion
                  callDelegate:(BOOL)aCallDelegate;

- (ETPopoverArrowDirection)arrowDirectionForRect:(CGRect)aRect
                                          inView:(UIView*)aView
                                     contentSize:(CGSize)aContentSize
                                     arrowHeight:(float)aArrowHeight
                        permittedArrowDirections:(ETPopoverArrowDirection)aArrowDirections;

- (CGSize)sizeForRect:(CGRect)aRect
               inView:(UIView *)aView
          arrowHeight:(float)aArrowHeight
       arrowDirection:(ETPopoverArrowDirection)aArrowDirection;
...
#pragma mark
#pragma mark - ETPopoverController
...
static ETPopoverTheme *defaultTheme_ = nil;

+ (void)setDefaultTheme:(ETPopoverTheme *)aTheme
{
    defaultTheme_ = aTheme;
    
    @autoreleasepool {
        ETPopoverBackgroundView *appearance = [ETPopoverBackgroundView appearance];
...
+ (void)load
{
    [ETPopoverController setDefaultTheme:[ETPopoverTheme theme]];
}
...
- (id)init
{
    ...
    if (self)
    {
        ...
        animationDuration = ET_POPOVER_DEFAULT_ANIMATION_DURATION;
        
        themeUpdatesEnabled = NO;
        
        [self setTheme:[ETPopoverController defaultTheme]];
...
- (void)setTheme:(ETPopoverTheme *)value
{
                    ...
                    completion:(void (^)(void))completion
{
    [self presentPopoverFromRect:aRect
        ...
        permittedArrowDirections:aArrowDirections
                        animated:aAnimated
                         options:ETPopoverAnimationOptionFade
...
- (void)presentPopoverFromRect:(CGRect)aRect
      ...
      permittedArrowDirections:(ETPopoverArrowDirection)aArrowDirections
                    ...
                    completion:(void (^)(void))completion
{
    NSAssert((aArrowDirections != ETPopoverArrowDirectionUnknown), @"ETPopoverArrowDirection must not be UNKNOWN");
    ...
    if (overlayView == nil)
    {
        overlayView = [[ETPopoverOverlayView alloc] initWithFrame:inView.window.bounds];
        ...
        backgroundView = [[ETPopoverBackgroundView alloc] initWithContentSize:contentViewSize];
    ...
    if (animated)
    {
        if ((options & ETPopoverAnimationOptionFade) == ETPopoverAnimationOptionFade)
        {
            overlayView.alpha = 0;
        ...
        if ((options & ETPopoverAnimationOptionScale) == ETPopoverAnimationOptionScale)
        {
                             ...
                             completion:(void (^)(void))completion
{
    ...
    aArrowDirections = ETPopoverArrowDirectionDown | ETPopoverArrowDirectionUp;
                            ...
                            completion:(void (^)(void))completion
{
    [self presentPopoverFromRect:CGRectZero
                          inView:nil
        permittedArrowDirections:ETPopoverArrowDirectionNone
                        animated:aAnimated
                         options:aOptions
                      completion:completion];
}

- (CGAffineTransform)transformForArrowDirection:(ETPopoverArrowDirection)arrowDirection
{
...
- (void)positionPopover:(BOOL)aAnimated
{
    ...
    CGSize contentViewSize = self.popoverContentSize;
    CGSize minContainerSize = ET_POPOVER_MIN_SIZE;
    ...
    ETPopoverArrowDirection arrowDirection = permittedArrowDirections;
    ...
    viewFrame = ETRectInWindowBounds(viewFrame, orientation);
    
    minX = popoverLayoutMargins.left;
    maxX = overlayWidth - popoverLayoutMargins.right;
    minY = ETStatusBarHeight() + popoverLayoutMargins.top;
    ...
    if (arrowDirection == ETPopoverArrowDirectionDown)
    {
        backgroundView.arrowDirection = ETPopoverArrowDirectionDown;
    ...
    if (arrowDirection == ETPopoverArrowDirectionUp)
    {
        backgroundView.arrowDirection = ETPopoverArrowDirectionUp;
        containerViewSize = [backgroundView sizeThatFits:contentViewSize];
    ...
    if (arrowDirection == ETPopoverArrowDirectionRight)
    {
        backgroundView.arrowDirection = ETPopoverArrowDirectionRight;
        containerViewSize = [backgroundView sizeThatFits:contentViewSize];
    ...
    if (arrowDirection == ETPopoverArrowDirectionLeft)
    {
        backgroundView.arrowDirection = ETPopoverArrowDirectionLeft;
        containerViewSize = [backgroundView sizeThatFits:contentViewSize];
    ...
    if (arrowDirection == ETPopoverArrowDirectionNone)
    {
        backgroundView.arrowDirection = ETPopoverArrowDirectionNone;
        containerViewSize = [backgroundView sizeThatFits:contentViewSize];
    ...
    backgroundView.transform = CGAffineTransformMakeRotation(ETInterfaceOrientationAngleOfOrientation(orientation));
    
    containerFrame = backgroundView.frame;
    
    containerFrame.origin = ETPointRelativeToOrientation(containerOrigin, containerFrame.size, orientation);
    ...
    ET_LOG(@"popoverContainerView.frame = %@", NSStringFromCGRect(backgroundView.frame));
}
                  ...
                  callDelegate:(BOOL)callDelegate
{
    ...
    ETPopoverAnimationOptions style = aOptions;
    ...
    if (aAnimated)
    {
        [UIView animateWithDuration:duration animations:^{
            ...
            if (strongSelf)
            {
                if ((style & ETPopoverAnimationOptionFade) == ETPopoverAnimationOptionFade)
                {
                    strongSelf->backgroundView.alpha = 0;
                }
                
                if ((style & ETPopoverAnimationOptionScale) == ETPopoverAnimationOptionScale)
                {
...
- (void)popoverOverlayViewDidTouch:(ETPopoverOverlayView *)aOverlayView
{
...
- (void)popoverBackgroundViewDidTouchOutside:(ETPopoverBackgroundView *)aBackgroundView
{
...
{
    ETPopoverArrowDirection arrowDirection = ETPopoverArrowDirectionUnknown;
    
    NSMutableArray *areas = [NSMutableArray arrayWithCapacity:0];
    ETPopoverArea *area;
    
    if ((arrowDirections & ETPopoverArrowDirectionDown) == ETPopoverArrowDirectionDown)
    {
        area = [[ETPopoverArea alloc] init];
        area.areaSize = [self sizeForRect:aRect inView:aView arrowHeight:arrowHeight arrowDirection:ETPopoverArrowDirectionDown];
        area.arrowDirection = ETPopoverArrowDirectionDown;
        [areas addObject:area];
    }
    
    if ((arrowDirections & ETPopoverArrowDirectionUp) == ETPopoverArrowDirectionUp)
    {
        area = [[ETPopoverArea alloc] init];
        area.areaSize = [self sizeForRect:aRect inView:aView arrowHeight:arrowHeight arrowDirection:ETPopoverArrowDirectionUp];
        area.arrowDirection = ETPopoverArrowDirectionUp;
        [areas addObject:area];
    }
    
    if ((arrowDirections & ETPopoverArrowDirectionLeft) == ETPopoverArrowDirectionLeft)
    {
        area = [[ETPopoverArea alloc] init];
        area.areaSize = [self sizeForRect:aRect inView:aView arrowHeight:arrowHeight arrowDirection:ETPopoverArrowDirectionLeft];
        area.arrowDirection = ETPopoverArrowDirectionLeft;
        [areas addObject:area];
    }
    
    if ((arrowDirections & ETPopoverArrowDirectionRight) == ETPopoverArrowDirectionRight)
    {
        area = [[ETPopoverArea alloc] init];
        area.areaSize = [self sizeForRect:aRect inView:aView arrowHeight:arrowHeight arrowDirection:ETPopoverArrowDirectionRight];
        area.arrowDirection = ETPopoverArrowDirectionRight;
        [areas addObject:area];
    }
    
    if ((arrowDirections & ETPopoverArrowDirectionNone) == ETPopoverArrowDirectionNone)
    {
        area = [[ETPopoverArea alloc] init];
        area.areaSize = [self sizeForRect:aRect inView:aView arrowHeight:arrowHeight arrowDirection:ETPopoverArrowDirectionNone];
        area.arrowDirection = ETPopoverArrowDirectionNone;
        [areas addObject:area];
    }
    ...
    if ([areas count] > 1)
    {
        NSIndexSet* indexes = [areas indexesOfObjectsPassingTest:^BOOL(id obj, NSUInteger idx, BOOL *stop) {
            ETPopoverArea* popoverArea = (ETPopoverArea*)obj;
    ...
    [areas sortUsingComparator:^NSComparisonResult(id obj1, id obj2) {
        ETPopoverArea *area1 = (ETPopoverArea *)obj1;
        ETPopoverArea *area2 = (ETPopoverArea *)obj2;
    ...
    {
        ETPopoverArea *popoverArea = (ETPopoverArea *)[areas objectAtIndex:i];
    ...
    if (arrowDirection == ETPopoverArrowDirectionUnknown)
    {
        if ([areas count] > 0)
        {
            arrowDirection = ((ETPopoverArea *)[areas objectAtIndex:0]).arrowDirection;
        }
        else
        {
            if ((arrowDirections & ETPopoverArrowDirectionDown) == ETPopoverArrowDirectionDown)
            {
                arrowDirection = ETPopoverArrowDirectionDown;
            }
            else if ((arrowDirections & ETPopoverArrowDirectionUp) == ETPopoverArrowDirectionUp)
            {
                arrowDirection = ETPopoverArrowDirectionUp;
            }
            else if ((arrowDirections & ETPopoverArrowDirectionLeft) == ETPopoverArrowDirectionLeft)
            {
                arrowDirection = ETPopoverArrowDirectionLeft;
            }
            else
            {
                arrowDirection = ETPopoverArrowDirectionRight;
            }
        }
    }
...
- (CGSize)sizeForRect:(CGRect)aRect
               inView:(UIView *)aView
          arrowHeight:(float)arrowHeight
       arrowDirection:(ETPopoverArrowDirection)arrowDirection
{
    UIInterfaceOrientation orientation = [[UIApplication sharedApplication] statusBarOrientation];
    ...
    else if (arrowDirection == ETPopoverArrowDirectionUp)
    {
        result.width = maxX - minX;
...
static NSString* StringFromOrientation(NSInteger orientation) {
...
static float ETStatusBarHeight() {
...
static float ETInterfaceOrientationAngleOfOrientation(UIInterfaceOrientation orientation)
{
    float angle;
...
static CGRect ETRectInWindowBounds(CGRect rect, UIInterfaceOrientation orientation) {
...
static CGPoint ETPointRelativeToOrientation(CGPoint origin, CGSize size, UIInterfaceOrientation orientation) {
...
- (void)keyboardWillShow:(NSNotification *)notification
{
    ...
    //UIInterfaceOrientation orientation = [[UIApplication sharedApplication] statusBarOrientation];
    //ET_LOG(@"orientation = %@", ETStringFromOrientation(orientation));
    //ET_LOG(@"keyboardRect = %@", NSStringFromCGRect(keyboardRect));
