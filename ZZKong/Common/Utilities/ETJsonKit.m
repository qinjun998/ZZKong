...
+ (NSString *)stringFromJSONObject:(id)aObj options:(NSJSONWritingOptions)option
{
    ...
    if ([NSJSONSerialization isValidJSONObject:aObj]) {
        ...
        if (!error) {
            ...
            ETLogE(@"error convert to json,%@,%@",error,aObj);
        }
    }
...
+ (NSDictionary *)dictFromString:(NSString *)aString
{
    ...
    if (error) {
        ETLogE(@"error convert json string to dict,%@,%@", aString, error);
        return nil;
    }
    else {
        return resultDict;
...
+ (NSArray *)arrayFromString:(NSString *)aString
{
    ...
    if (error) {
        ETLogE(@"error convert json string to array,%@,%@", aString, error);
        return nil;
    } else {
        return resultArray;
    }
